...
class OverViewPage extends BasePage {
  ...
  async totalBTCIsOrange() {
    ...
    const totalBTCLocator = this.page.locator(
      this.selectors.totalBTCDucatValuts,
    );
  ...
  async collateralizationRatioIsOrange() {
    ...
    const ratioLocator = this.page.locator(
      this.selectors.collateraizationRatio,
    );
  ...
  async verifyAllOptionsVisible() {
    ...
    if (!allVisible) {
      const missingElements = Object.entries(results)
        ...
        .join(", ");
      throw new Error(
        `Not all elements are visible. Missing: ${missingElements}`,
      );
    }
  ...
  async getPercentageValue() {
    const percentageRangeLocator = this.page.locator(
      this.itemscenter.precntageRange,
    );
  ...
  async cycleAndVerifyTimeRanges() {
    ...
    if (uniqueTimeRanges.size !== 4) {
      throw new Error(
        `Expected 4 unique time ranges, but found ${uniqueTimeRanges.size}`,
      );
    }

    if (currentTimeRange !== initialTimeRange) {
      throw new Error(
        `Did not return to initial time range after full cycle. Expected ${initialTimeRange}, got ${currentTimeRange}`,
      );
    }
    ...
    try {
      ...
      for (const [key, selector] of Object.entries(this.transactiontable)) {
        try {
          ...
          results[key] = false;
          console.error(
            `Transaction table header "${key}" is not visible: ${error.message}`,
          );
        }
      }
      const allVisible = Object.values(results).every((isVisible) => isVisible);
      ...
      if (!allVisible) {
        const missingHeaders = Object.entries(results)
          ...
          .join(", ");
        throw new Error(
          `Not all transaction table headers are visible. Missing: ${missingHeaders}`,
        );
      }
      ...
      throw new Error(
        `Transaction table verification failed: ${error.message}`,
      );
    }
  }
}
