...
class VaultCreationPage extends BasePage {
  ...
  async completeVaultCreation() {
    let errorExists = await this.page.locator(this.selectors.feeErrorMessage)
    ...
    if (errorExists) {
      ...
      while (Date.now() - startTime < maxWaitTime) {
        errorExists = await this.page.locator(this.selectors.feeErrorMessage)
          .filter({ hasText: "The fees to open a vault are higher than your balance" })
          .isVisible()
          .catch(() => false);
  ...
  async handleConfirmation(context, handlePageTransition) {
    ...
    if (confirmationPage) {
      try {
        await confirmationPage.waitForSelector(this.selectors.confirmAllButton, {
          state: 'visible',
          timeout: 15000
        });
