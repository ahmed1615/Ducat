...
When('I connect my wallet in the first time', async function() {
  ...
  const walletPage = await this.appPage.selectXverseWallet(this.context, handlePageTransition);
  if (walletPage) {
...
Then('the error network selection do not match', async function() {
   ...
   expect(errorMessage).toContain('Your wallet address and network selection don');
  });

When('I configure the network in Xverse', async function() {
  const xverseConfigPage = new XverseExtensionPage(await this.context.newPage());
...
When('I reconnect to Ducat Protocol', async function() {
  ...
  const walletPage2 = await appPage2.selectXverseWallet(this.context, handlePageTransition);
  if (walletPage2) {
...
When('I open Xverse extension and navigate to Ducat Protocol app', async function() {
  try {
    ...
    const walletPage = await appPage.selectXverseWallet(context, handlePageTransition);
    if (walletPage) {
      await walletPage.click('xpath=//*[contains(text(), "Accept")]');
    } else {
      console.error('Failed to capture the wallet page');
    ...
    const walletPage2 = await appPage2.selectXverseWallet(context, handlePageTransition);
    if (walletPage2) {
      await walletPage2.click('xpath=//*[contains(text(), "Accept")]');
    } else {
      console.error('Failed to capture the wallet page');
